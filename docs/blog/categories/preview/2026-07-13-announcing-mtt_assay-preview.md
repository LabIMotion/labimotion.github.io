...
The new **MTT Assay** workflow takes **Viability** (new Generic Element) wellplate data through a full **dose-response analysis** and brings the results back into the host app (e.g. ELN). From a Viability's **Wellplates** tab you submit selected wellplates for processing, track each request's status, review the **computed endpoint (e.g. IC50)** and **dose-response curves**, and then send the analyses you trust back to the corresponding **samples** as measurements.
...
>
> **Interface:** Chemotion ELN / Generic Element: Viability – Wellplates & Measurements tabs
{: .info }
...
- **Results in the Measurements tab**: Each sample's **Measurements** tab lists its MTT analyses grouped by Generic Element, with IC50 / pIC50 metrics, a link back to the originating Generic Element, and per-row delete.
