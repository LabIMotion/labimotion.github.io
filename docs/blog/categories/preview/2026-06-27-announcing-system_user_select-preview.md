...
- A user who **no longer exists** (e.g. deleted) shows as **`<id> (depr.)`** so the selection stays visible rather than silently vanishing.
...
- **Orphans don't disappear.** An id that resolves to nothing falls back to `<id> (depr.)` rather than being dropped, keeping the selection intact.
